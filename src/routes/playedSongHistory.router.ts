...
router.get(
  ...
  async (
    request: Request,
    response: Response<PlayedSongHistoryDto[]>,
    next: NextFunction
  ) => {
    try {
      ...
      const playedSongHistoryRepo = getCustomRepository(
        PlayedSongHistoryRepository
      );

      const theLastTenSong = await playedSongHistoryRepo.getLastPlayedByUser(
        userId,
        limit
      );
    ...
    try {
      ...
      const recommendedSongs = await (async () => {
        if (_.isEmpty(theLastTenSong)) {
          ...
          return playedSongHistoryRepo.getByCategoriesAndOrderByTime(
            mostPlayedCategory
          );
        }
      })();
